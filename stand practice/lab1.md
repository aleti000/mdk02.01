...
4. **CLI-office** - получает любой свободный адрес из пула адресов office(**DHCP, настройка не требуется!!!**)

**Критерии выполнения работы:**
Работа считается выполненной если SRV1 и CLI-home могут пинговать интернет-адреса по именам (доменные имена успешно разрешаются в IP-адреса).
